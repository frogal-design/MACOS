...
// Hoisted static data to module scope to avoid re-creation on every render
const CATEGORIES = ['all', 'events', 'sports', 'campus', 'clubs', 'leadership'] as const;

// Pre-sorted media array at module level to eliminate O(n log n) sorting overhead during render
const MEDIA = [
  {
    id: 1,
    type: 'image',
...
const SORTED_MEDIA = [...MEDIA].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
...
const Gallery = () => {
  ...
  // Memoized filter results to prevent unnecessary filtering on every re-render
  const filteredMedia = useMemo(() => {
    return filter === 'all' ? SORTED_MEDIA : SORTED_MEDIA.filter(m => m.category === filter);
  }, [filter]);

  // Derived state for the selected item reduces redundancy and synchronization bugs
