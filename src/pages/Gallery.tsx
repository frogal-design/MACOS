 ...
 * 1. Hoisted MEDIA array and CATEGORIES to module scope to prevent re-allocation on every render.
 * 2. Pre-sorted MEDIA by date (O(n log n) once) instead of on every render cycle.
 * 3. Wrapped filteredMedia in useMemo (O(n) on filter change) to avoid redundant computations.
 * 4. Memoized event handlers and callbacks with useCallback.
 * 5. Added aria-labels to icon-only buttons for accessibility and reliable testing.
 */
...
] as const satisfies MediaItem[]).slice().sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
            ...
            <div className="flex gap-10">
              {CATEGORIES.map(cat => (
                <button
                  ...
                  onClick={() => {
                    setFilter(cat);
                    setCurrentIndex(-1); // Reset modal if filter changes
                  }}
