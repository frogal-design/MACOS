...
const SORTED_MEDIA_DATA = [...MEDIA_DATA].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
...
const Gallery = () => {
  ...
  const filteredMedia = React.useMemo(() =>
    (filter === 'all' ? SORTED_MEDIA_DATA : SORTED_MEDIA_DATA.filter(m => m.category === filter)),
    [filter]
  );

  const openModal = React.useCallback((item: MediaItem, index: number) => {
