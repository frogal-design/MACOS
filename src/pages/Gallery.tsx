...
// Hoist static data to avoid re-creation on every render
...
const Gallery = () => {
  ...
  // Memoize filtered media to avoid re-sorting on every render
  const filteredMedia = React.useMemo(() => {
    const base = filter === 'all' ? MEDIA : MEDIA.filter(m => m.category === filter);
    return [...base].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [filter]);

  // Derived state: the currently selected media item
  const selectedMedia = currentIndex >= 0 ? filteredMedia[currentIndex] : null;

  const openModal = React.useCallback((_item: MediaItem, index: number) => {
  ...
  const navigateModal = React.useCallback((direction: 'prev' | 'next') => {
