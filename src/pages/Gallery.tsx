...
const Gallery = () => {
  ...
  // Derived state for the selected media item to avoid redundant state updates
  const selectedMedia = useMemo(() => {
    return currentIndex !== null ? filteredMedia[currentIndex] : null;
  ...
  // Global keyboard listeners: only bound when modal is open to avoid re-binding during active navigation
  useEffect(() => {
    if (!isOpen) return;
