...
import { ArrowLeft, Play, Filter, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
/**
 * Static categories hoisted to module scope to prevent re-creation on every render.
 */
const CATEGORIES = ['all', 'events', 'sports', 'campus', 'clubs', 'leadership'];

/**
 * Static media data hoisted to module scope.
 * Pre-sorted by date descending to eliminate O(n log n) sorting overhead during render.
 */
const MEDIA = [
  {
    id: 1,
...
/**
 * Pre-sorted media array to eliminate O(n log n) sorting overhead during render.
 */
const SORTED_MEDIA = [...MEDIA].sort((a, b) => (b.date > a.date ? 1 : b.date < a.date ? -1 : 0));
...
const Gallery = () => {
  ...
  // Memoized filtered media list to prevent re-filtering on every render.
  const filteredMedia = useMemo(() => {
    return filter === 'all' ? SORTED_MEDIA : SORTED_MEDIA.filter(m => m.category === filter);
  }, [filter]);

  // Derive selectedMedia from currentIndex and filteredMedia to eliminate redundant state.
  const selectedMedia = currentIndex >= 0 && currentIndex < filteredMedia.length
