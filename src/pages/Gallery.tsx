...
/**
 * Static media data hoisted to module scope and pre-sorted by date (descending).
 * Pre-sorting at the module level removes O(n log n) overhead from the React render cycle.
 */
const MEDIA = [
  {
    id: 1,
...
const Gallery = () => {
  ...
  // Memoize filtered media to reduce operation to O(n) filtering.
  const filteredMedia = useMemo(() =>
    filter === 'all' ? SORTED_MEDIA : SORTED_MEDIA.filter(m => m.category === filter),
  [filter]);

  // Derive selectedMedia from currentIndex to prevent state synchronization bugs.
