...
// Static media data hoisted to module scope to avoid re-creation on every render.
// The array is pre-sorted by date (newest first) to optimize the filtering logic.
...
] as const).slice().sort((a, b) => b.date.localeCompare(a.date));
...
const Gallery = () => {
  ...
  // Memoize filtered results to prevent expensive re-filtering/sorting on every render
  // (e.g., when opening/closing the modal or navigating between items).
