...
// Pre-sort media by date descending at module level to save O(n log n) overhead on every render
...
const Gallery = () => {
  ...
  // Memoize filtered results to avoid recalculation unless filter changes
  const filteredMedia = useMemo(() => {
    return filter === 'all' ? SORTED_MEDIA : SORTED_MEDIA.filter(m => m.category === filter);
  }, [filter]);

  // Derive selectedMedia from currentIndex instead of storing whole object in state
