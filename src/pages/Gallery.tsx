...
// Hoist static categories to avoid re-allocation on each render
const CATEGORIES = ['all', 'events', 'sports', 'campus', 'clubs', 'leadership'];

// Hoist and pre-sort media items at module scope to move O(n log n) work
// out of the component's render cycle, ensuring efficient display.
...
] as const).slice().sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()) as MediaItem[];
...
const Gallery = () => {
  ...
  // Memoize filtered items to prevent redundant O(n) filtering when unrelated state changes.
  const filteredMedia = useMemo(() => {
    return filter === 'all' ? MEDIA : MEDIA.filter(m => m.category === filter);
  }, [filter]);

  // Derived state for the modal's selected item to maintain a single source of truth.
