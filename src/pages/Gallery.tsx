...
] as const as MediaItem[];

// Pre-sorting the static dataset once at module scope (O(n log n))
// This eliminates the overhead of sorting on every React render cycle.
const SORTED_MEDIA = [...MEDIA].sort((a, b) =>
...
const Gallery = () => {
  ...
  // O(n) filter memoized to only run when the filter state changes.
  const filteredMedia = useMemo(() =>
    filter === 'all' ? SORTED_MEDIA : SORTED_MEDIA.filter(m => m.category === filter),
  [filter]);

  // Derived state: selectedMedia is derived from currentIndex and the current filtered set.
  // This prevents synchronization bugs and reduces redundant state updates.
