...
// Hoisted constants to avoid re-allocation during renders
...
const Gallery = () => {
  ...
  // Memoize filtered and sorted results to avoid re-calculating on every render
  const filteredMedia = useMemo(() => {
    const items = filter === 'all' ? MEDIA : MEDIA.filter(m => m.category === filter);
    // Use string comparison for ISO dates to avoid expensive new Date() calls in sort
    return [...items].sort((a, b) => b.date > a.date ? 1 : b.date < a.date ? -1 : 0);
  }, [filter]);

  // Derive selectedMedia from state to avoid redundant state sync anti-patterns
