...
// Hoist static media data and pre-sort it by date (descending) at module level
// to avoid O(n log n) sorting overhead during every render.
...
const Gallery = () => {
  ...
  // Memoize filtered results to prevent expensive re-computations on every render.
  const filteredMedia = useMemo(() => {
    return filter === 'all' ? MEDIA : MEDIA.filter(m => m.category === filter);
  }, [filter]);

  // Derive selectedMedia from state to eliminate redundant state synchronization
  // and potential bugs when the underlying data changes.
