...
// ⚡ Bolt: Hoist static data to module scope to prevent re-allocation on every render.
...
// ⚡ Bolt: Pre-sort once at module scope.
...
const Gallery = () => {
  ...
  // ⚡ Bolt: Use useMemo for filtered results to avoid redundant computations on every render.
  const filteredMedia = useMemo(() => {
    return filter === 'all' ? SORTED_MEDIA : SORTED_MEDIA.filter(m => m.category === filter);
  }, [filter]);

  const openModal = useCallback((item: MediaItem, index: number) => {
  ...
  }, [selectedMedia, closeModal, navigateModal]);
              ...
              <div className="relative">
                <div className="aspect-[4/5] bg-bg overflow-hidden border border-border relative group/vid shadow-2xl">
                  <img 
                    ...
                    loading="lazy" // ⚡ Bolt: Add lazy loading to below-the-fold image.
                  />
