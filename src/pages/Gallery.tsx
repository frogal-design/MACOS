...
import { ArrowLeft, Play, Image as ImageIcon, Video, Filter, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
// Hoist static data to module scope to prevent re-allocation on every render.
const CATEGORIES = ['all', 'events', 'sports', 'campus', 'clubs', 'leadership'] as const;

// Pre-sort MEDIA by date at module scope (O(n log n) once) instead of during render.
...
] as const).slice().sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
...
const Gallery = () => {
  ...
  // Memoize filtered results to reduce work during re-renders to O(n) instead of O(n log n).
  const filteredMedia = useMemo(() =>
    filter === 'all' ? MEDIA : MEDIA.filter(m => m.category === filter),
  [filter]);

  // Derived state: Use index to select from filtered results.
  // This maintains a single source of truth and prevents state synchronization bugs.
  const selectedMedia = currentIndex !== -1 ? filteredMedia[currentIndex] : null;

  const openModal = useCallback((_item: MediaItem, index: number) => {
