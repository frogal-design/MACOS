...
/**
 * Static media data hoisted to module scope to prevent re-creation during render.
 * Pre-sorted by date in descending order (O(n log n) once) to remove sorting
 * overhead from the React component lifecycle.
 */
const MEDIA = [
  {
    id: 1,
...
const SORTED_MEDIA = [...MEDIA].sort((a, b) => b.date.localeCompare(a.date));
...
const Gallery = () => {
  ...
  /**
   * Filtered media list memoized to prevent redundant calculations.
   * Sorting is removed here as MEDIA is pre-sorted at module scope.
   */
  const filteredMedia = useMemo(() =>
    filter === 'all' ? SORTED_MEDIA : SORTED_MEDIA.filter(m => m.category === filter),
  [filter]);

  /**
   * Derived state for the selected media item based on the active index.
   * This eliminates redundant state synchronization and potential bugs.
   */
