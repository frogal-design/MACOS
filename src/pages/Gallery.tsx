...
] as const).slice().sort((a, b) => b.date.localeCompare(a.date)) as MediaItem[];
...
const Gallery = () => {
  ...
  // Memoize filtered results to avoid O(n) filtering on every render
  const filteredMedia = useMemo(() => {
    return filter === 'all' ? MEDIA : MEDIA.filter(m => m.category === filter);
  }, [filter]);

  // Derive selectedMedia from index and filteredMedia to prevent redundant state
  const selectedMedia = useMemo(() =>
    currentIndex >= 0 && currentIndex < filteredMedia.length ? filteredMedia[currentIndex] : null
              ...
              {filteredMedia.map((item, idx) => (
                ...
                >
                  <div className="absolute top-6 left-6 z-20 font-serif text-[9px] tracking-[0.4em] text-accent opacity-0 group-hover:opacity-100 transition-all duration-500 transform -translate-y-2 group-hover:translate-y-0 uppercase font-medium">
                    {item.category} • {item.date.split('-')[0]}
                  </div>
