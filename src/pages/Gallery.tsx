...
// Pre-sort media at module level by date (DESC) to save O(n log n) overhead during render
const MEDIA = [...RAW_MEDIA].sort((a, b) => (b.date > a.date ? 1 : b.date < a.date ? -1 : 0));
...
const Gallery = () => {
  ...
  // Memoize filtered results to prevent unnecessary computation on re-renders
  const filteredMedia = useMemo(() => {
    return filter === 'all' ? MEDIA : MEDIA.filter(m => m.category === filter);
  }, [filter]);

  // Derive selectedMedia from state to eliminate redundant synchronization
